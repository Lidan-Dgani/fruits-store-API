...
let message = (json, evt) => {
	console.log(json);
	if (evt.path[1].innerHTML.includes("watermelon")) {
		alert(`יש לנו הנחה של ${json.watermelon} על המוצר! `);
	} else if (evt.path[1].innerHTML.includes("banana")) {
		alert(`יש לנו הנחה של ${json.Banana} על המוצר! `);
	} else {
		alert(`מצטערים, אין לנו היום הנחה על המוצר הזה 🙏`);
	}
};
